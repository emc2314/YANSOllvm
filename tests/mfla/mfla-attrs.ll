...
; RUN: grep 'define internal i32 @pureish' %t.out.ll | grep -qv 'readnone'
; RUN: grep 'define internal i32 @pureish' %t.out.ll | grep -qv 'willreturn'
; RUN: grep 'define internal i32 @pureish' %t.out.ll | grep -qv 'mustprogress'
...
define internal i32 @pureish(i32 %x) #0 {
entry:
  %y = add i32 %x, 1
  ret i32 %y
}
