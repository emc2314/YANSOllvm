...
// RUN: %opt -load-pass-plugin %plugin -passes=yanso -verify-each -S %t/local-ssa.ll -o %t/local-ssa.obf.ll
// RUN: %clang %t/local-ssa.obf.ll -o %t/local-ssa.obf
// RUN: %t/local-ssa.obf > %t/local-ssa.stdout
// RUN: grep '^yanso-local-ssa:23$' %t/local-ssa.stdout
