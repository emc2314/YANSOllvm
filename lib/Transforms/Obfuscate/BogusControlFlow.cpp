...
static YansoRNG *BCFRNG = nullptr;
...
PreservedAnalyses BogusControlFlowPass::run(Function &F,
  ...
  if (toObfuscate(flag, &F, "bcf")) {
    YansoRNG LocalRNG(yanso_function_seed(F, "bcf"));
    RNG = &LocalRNG;
    BCFRNG = &LocalRNG;
    bogus(F);
    doF(*F.getParent(), F);
...
void BogusControlFlowPass::bogus(Function &F) {
  ...
  do {
    ...
    while (!basicBlocks.empty()) {
      ...
      if ((int)BCFRNG->range(100) <= ObfProbRate) {
...
void BogusControlFlowPass::addBogusFlow(BasicBlock *basicBlock, Function &F) {
  ...
  // them.
  BasicBlock::iterator i1 = basicBlock->getFirstNonPHIOrDbgOrLifetime();
  ...
  for (BasicBlock::iterator i = alteredBB->begin(), e = alteredBB->end();
    ...
    if (i->isBinaryOp()) { // binary instructions
      ...
      if (opcode == Instruction::Add || opcode == Instruction::Sub ||
        ...
        for (int random = (int)BCFRNG->range(10); random < 10; ++random) {
          switch (BCFRNG->range(4)) { // to improve
          case 0:                     // do nothing
            break;
          case 1:
      ...
      if (opcode == Instruction::FAdd || opcode == Instruction::FSub ||
        ...
        for (int random = (int)BCFRNG->range(10); random < 10; ++random) {
          switch (BCFRNG->range(3)) { // can be improved
          case 0:                     // do nothing
            break;
          case 1:
      ...
      if (opcode == Instruction::ICmp) { // Condition (with int)
        ...
        switch (BCFRNG->range(3)) { // must be improved
        case 0:                     // do nothing
          break;
        case 1:
...
bool BogusControlFlowPass::doF(Module &M, Function &F) {
  ...
  for (std::vector<Instruction *>::iterator i = toEdit.begin();
       i != toEdit.end(); ++i) {
    // if y < 10 || x*(x+1) % 2 == 0
    opX = new LoadInst(Type::getInt32Ty(M.getContext()), (Value *)x, "",
                       (*i)->getIterator());
    opY = new LoadInst(Type::getInt32Ty(M.getContext()), (Value *)y, "",
                       (*i)->getIterator());
